    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (data.Item) {
                ...
                await interaction.reply(`퐁! 유저 ID: ${userId}, ${translateMessage}, 설정된 언어: ${setLang}`);
            } else {
                await interaction.reply(`퐁! 유저 데이터를 찾을 수 없어요.`);
            }
        } catch (error) {
            console.error('DynamoDB 조회 오류: ', error);
            await interaction.reply('데이터를 가져오는 도중 오류가 발생했어요.');
        }
    },
};
